\begin{frame}
\frametitle{Experimentaci\'on}
Se van a realizar 2 experimentos para comprobar esto: \\
\vspace{3mm}
\begin{itemize}
\item Dos 2 modelos clasificadores en paralelo
\item Fusionador OR/AND l\'ogico 
\item Se simulan con variables aleatorias los resultados de clasificaci\'on de cada modelo
\end{itemize}
\end{frame}



\begin{frame}
\frametitle{Experimentaci\'on}
\begin{center}
\begin{tikzpicture}[
every node/.style={font=\small},
datos/.style={draw, fill=red!10, minimum width=1.7cm, minimum height=1cm},
modelo/.style={draw, fill=blue!10, minimum width=1.7cm, minimum height=1cm},
fusionador/.style={draw, fill=green!10, minimum width=6cm, minimum height=1cm},
sistema/.style={draw, fill=gray!5}
]
% Datos
\node[datos] (datos1) at (-2,3) {Im\'agenes};
\node[datos] (datos2) at (2,3) {Historia};
% Sistema
\node[sistema, minimum width=8cm, minimum height=4cm] (sistema) at (0,0) {};
% Modelos
\node[modelo] (modelo1) at (-2,1) {Modelo 1};
\node[modelo] (modelo2) at (2,1) {Modelo 2};
% Fusionador
\node[fusionador] (fusionador) at (0,-1) {Fusionador};
% Salida
\node[datos] (resultado) at (0,-3) {Predicci\'on};
% Aristas
\draw[->] (datos1) to (modelo1);
\draw[->] (datos2) to (modelo2);
\draw[->] (modelo1) to (fusionador);
\draw[->] (modelo2) to (fusionador);
\draw[->] (fusionador) to (resultado);
\end{tikzpicture}
\end{center}
\end{frame}



\begin{frame}
\frametitle{Metodolog\'ia}
Para ambos experimentos la operaci\'on es la siguiente: \\
\vspace{3mm}
\begin{itemize}
\item Se simula y guarda el resultado de la intervenci\'on en un dataset
\item Se simula el resultado del modelo para otro dataset
\item Se calcula la funci\'on de costo de peor caso estimando las confusiones de intervenci\'on
\item Se ejecuta el sistema completo en el dataset del modelo para comprobar los resultados
\end{itemize}
\end{frame}



\begin{frame}
\frametitle{Experimento 1}
\begin{itemize}
\item Los accuracies de ambos modelos 0.5 ambas clases
\item Fusionador de tipo OR
\item Correlaci\'on clase Positive de 0.95
\item Correlaci\'on clase Negative de -0.95
\end{itemize}
\vspace{3mm}
Se asignan distintos tamaños en cada dataset y para cada combinaci\'on se ejecuta 1000 veces y se reporta: \\
\vspace{3mm}
\begin{itemize}
\item $P(PeorCasoEstimado \geq EvalReal)$
\end{itemize}
\end{frame}



\begin{frame}
\frametitle{Experimento 1}
\centering
\includegraphics[height=0.8\paperheight]{Imgs/fijo_1000.png}
\end{frame}

\begin{frame}
\frametitle{Experimento 1}
La probabilidad se acerca r\'apidamente a 1 a medida que ambas cantidades de instancias aumentan. \\
\vspace{3mm}
Para el caso de un sistema con dos modelos, a tamaños no tan grandes, el peor caso usando datasets distintos provee casi la misma informaci\'on que con un solo dataset.
\end{frame}



\begin{frame}
\frametitle{Experimento 2}
\begin{itemize}
    \item Accuracies de ambos modelos aleatorias
    \item Fusionador aleatorio entre OR y AND
    \item Correlaci\'ones aleatorias v\'alidas para los accuracies
\end{itemize}
\vspace{3mm}
Los tamaños de ambos datasets tienen 10000 instancias por clase, se ejecuta 100000 veces y se reporta: \\
\vspace{3mm}
{\fontsize{17}{17}\selectfont
\begin{itemize}
\item $\frac{EvalReal}{PeorCasoEstimado}$
\end{itemize}
}
\end{frame}



\begin{frame}
\frametitle{Experimento 2}
\centering
\includegraphics[height=0.8\paperheight]{Imgs/random_100000_10000.png}
\end{frame}



\begin{frame}
\frametitle{Experimento 2}
El cociente entre ambos valores no suele pasar 1, siendo esto el resultado buscado. Incluso en los raros casos en los que si lo supera, no lo hace por un m\'argen muy grande.
\end{frame}



\begin{frame}
\frametitle{Explicaci\'on}
La causa de estos resultados es que ambos datasets tienen datos provenientes de la misma distribuci\'on. \\
\vspace{3mm}
Por ley de los grandes n\'umeros, a medida que aumenta la cantidad de instancias de cada dataset se puede saber lo siguiente: \\
\vspace{3mm}
\begin{itemize}
    \item El accuracy calculado para el dataset del sistema va a tender al accuracy de la poblaci\'on
    \item Si se ejecutaba el sistema intervenido con los datos del modelo su accuracy hubiera tenidodo a la media de la poblaci\'on
\end{itemize}
\end{frame}



\begin{frame}
\frametitle{Explicaci\'on}
Como ambos accuracies tienden a lo mismo, los c\'alculos van a ser correctos:
\vspace{3mm}
\begin{itemize}
\item $\overline{\text{$m_{FN}^{S_{*\rightarrow p}, D}$}}$ $\xlongrightarrow{p}$ \IPFN
\item $\overline{\text{$m_{TP}^{S_{*\rightarrow p}, D}$}}$ $\xlongrightarrow{p}$ \IPTP
\item $\overline{\text{$m_{TP}^{S_{*\rightarrow n}, D}$}}$ $\xlongrightarrow{p}$ \INTP
\end{itemize}
\end{frame}