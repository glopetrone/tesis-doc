...
Una posible utilidad de esta funci\'on de costo puede ser realizar b\'usqueda de hiperpar\'ametros. \\
\vspace{3mm}
Se puede querer reemplazar o añadir un modelo al sistema. Al momento de realizar la b\'usqueda de hiperpar\'ametros para ese nuevo modelo, con esta funci\'on de costo se puede comparar entre modelos de forma mucho mas r\'apida y as\'i elegir a los m\'as adecuados, al no tener que ejecutar el sistema para cada modelo.
\end{frame}
...
\vspace{3mm}
Luego se podr\'ia calcular la matriz de confusi\'on asociada a sus resultados.
\end{frame}
...
\node[sistema, minimum width=8cm, minimum height=4cm] (sistema) at (0,0) {};
% Modelos
\node[modelo, fill=green!75] (modelo1) at (-2,1) {Positive};
\node[modelo] (modelo2) at (2,1) {Modelo 2};
% Fusionador
\node[fusionador] (fusionador) at (0,-1) {Fusionador};
% Salida
...
\item La m\'etrica a evaluar va a ser, dado un sistema y dada la matriz de confusi\'on de un modelo, calcular una cota al costo del sistema, es decir, el peor caso del sistema dado que tenga al modelo como componente.
\end{itemize}
\end{frame}